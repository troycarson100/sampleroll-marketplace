...
- **Stripe** — see **[`docs/STRIPE_QUICK_SETUP.md`](docs/STRIPE_QUICK_SETUP.md)** (keys, webhooks, Connect, local `stripe:listen`). Connect payout fields: [`docs/STRIPE_CONNECT.md`](docs/STRIPE_CONNECT.md).
- **Platform / contracts** — set **`PLATFORM_OWNER_EMAILS`** (comma-separated) for `/platform` (revenue-share contracts, PDF upload, creator approval). See **[`docs/PLATFORM_CONTRACTS.md`](docs/PLATFORM_CONTRACTS.md)**. Optional **Resend** envs for invitation email.
- **Never commit** `.env.local` (it is gitignored).

### MCP (optional): Stitch + Supabase + Stripe
...
- **Stripe MCP** — add the official Stripe MCP in **Cursor Settings → MCP** (uses your Stripe secret key). It helps with account info and docs; **webhooks still must be created** in the Dashboard or via **Stripe CLI** (`npm run stripe:listen`).

## Scripts

| Command        | Description                    |
|----------------|--------------------------------|
| `npm run db:ping` | Prisma `SELECT 1` using `DATABASE_URL` from `.env.local` (checks connectivity; needs a `dotenv` CLI that supports `-e`; if that fails, set `DATABASE_URL` in the shell and run `printf 'SELECT 1;\n' \| npx prisma db execute --stdin`) |
